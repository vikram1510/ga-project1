...
function setupPacman() {
  ...
  for (let i = 0; i < width ** 2; i++) {
    ...
    if (gameMode){
      if (wallArray.includes(i)) cell.classList.add('dot')
...
function gameStart(keyCode){
  ...
  if (stage.includes('Menu')) {
    ...
    if (keyCode === 37) {
      ...
      if (difficultyIndex - 1 < 0) difficultyIndex = 0
      ...
      if (difficultyIndex + 1 > difficulties.length - 1) difficultyIndex = difficulties.length - 1
      else difficultyIndex += 1
